...
void main() {
	...
	const string SERVER_NAME = "Hello"s;

	SOCKADDR_IN serv;
	int sizeS = sizeof(serv);
	...
	try
	{
		...
		if (GetServer((char*)SERVER_NAME.c_str(), cC, (SOCKADDR*) & serv, &sizeS))
		{
			cout << "Сервер получен: " << inet_ntoa(serv.sin_addr) << ":" << htons(serv.sin_port) << endl;
		}
		else
		{
			cout << "Сервер не получен!";
		}
