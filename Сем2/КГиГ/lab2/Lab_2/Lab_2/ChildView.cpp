...
BEGIN_MESSAGE_MAP(CChildView, CWnd)
	...
	ON_COMMAND(ID_LABBLADE_BLADE, &CChildView::OnLabBlade)
END_MESSAGE_MAP()
...
{
	double Xl = -3 * _PI;		// Координата Х левого угла области
	double Xh = -Xl;			// Координата Х правого угла области
	double dX = _PI / 36;		// Шаг графика функции
...
void CChildView::RotateBlade(double h, double fi, double R)
{
	CPaintDC dc(this);
	double step = 1.0; // 1 градус

	PenLine.Set(PS_SOLID, 1, RGB(255, 0, 0));
	PenAxis.Set(PS_SOLID, 2, RGB(255, 0, 0));

	RW.SetRect(0, 0, 200, 200);
	CPlot2D blade;
	...
	matrix(1) = 4;
	matrix(2) = 65;
	blade.SetParams(matrix, matrix, RW);

	blade.Draw(dc, 1, 1);
}

void CChildView::OnLabBlade()
{
